...
dnf install -y --disablerepo='updates*' git python3 python3-pip python3-virtualenv polkit yad

python3 -m venv /opt/potos-firstboot/venv
# Hash-pinned (firstboot/requirements.txt)
/opt/potos-firstboot/venv/bin/pip install --require-hashes -r "${SETUP_DIR}/requirements.txt"
...
if [ -f "${SETUP_DIR}/splash.png" ]; then
  install -d -m 0755 -o root -g root /boot/grub2
  install -m 0644 -o root -g root "${SETUP_DIR}/splash.png" /boot/grub2/splash.png
  if grep -q '^GRUB_BACKGROUND=' /etc/default/grub 2>/dev/null; then
    sed -i 's#^GRUB_BACKGROUND=.*#GRUB_BACKGROUND="/boot/grub2/splash.png"#' /etc/default/grub
  else
    echo 'GRUB_BACKGROUND="/boot/grub2/splash.png"' >>/etc/default/grub
  fi
...
# Success: firstboot.on_success = logout
# End the session that ran the wizard so the next login starts clean.
# PKEXEC_UID is set by pkexec to the UID of the invoking (logged-in) user;
# skip when absent (e.g. wrapper run manually as root).
if [ -n "${PKEXEC_UID:-}" ]; then
  loginctl terminate-user "${PKEXEC_UID}"
fi
